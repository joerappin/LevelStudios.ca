...
const STUDIO_PALETTE = [
  { bg: '#3B82F6', light: 'rgba(59,130,246,0.15)',  text: '#93C5FD', border: 'rgba(59,130,246,0.35)' },
  { bg: '#10B981', light: 'rgba(16,185,129,0.15)',  text: '#6EE7B7', border: 'rgba(16,185,129,0.35)' },
  { bg: '#8B5CF6', light: 'rgba(139,92,246,0.15)',  text: '#C4B5FD', border: 'rgba(139,92,246,0.35)' },
  { bg: '#F59E0B', light: 'rgba(245,158,11,0.15)',  text: '#FCD34D', border: 'rgba(245,158,11,0.35)' },
  { bg: '#EC4899', light: 'rgba(236,72,153,0.15)',  text: '#F9A8D4', border: 'rgba(236,72,153,0.35)' },
  { bg: '#14B8A6', light: 'rgba(20,184,166,0.15)',  text: '#5EEAD4', border: 'rgba(20,184,166,0.35)' },
  { bg: '#6366F1', light: 'rgba(99,102,241,0.15)',  text: '#A5B4FC', border: 'rgba(99,102,241,0.35)' },
  { bg: '#F97316', light: 'rgba(249,115,22,0.15)',  text: '#FDBA74', border: 'rgba(249,115,22,0.35)' },
]
...
export default function StudioCalendar({ reservations = [], showClientDetails = true, onDelete, onUpdate, onDayDoubleClick }) {
  ...
  const studioColors = useStudioColors(studios.slice(1))
  ...
  const headerLabel = useMemo(() => {
    ...
    if (calView === 'day')   return `${DAYS_LONG[(cursor.getDay()+6)%7]} ${cursor.getDate()} ${MONTHS[month]} ${year}`
    const days = getWeekDays(cursor)
    const s = days[0], e = days[6]
    if (s.getMonth() === e.getMonth())
      return `${s.getDate()}–${e.getDate()} ${MONTHS[s.getMonth()]} ${s.getFullYear()}`
    return `${s.getDate()} ${MONTHS_S[s.getMonth()]} – ${e.getDate()} ${MONTHS_S[e.getMonth()]} ${e.getFullYear()}`
  }, [calView, cursor])

  function getDayResas(date) {
    if (!date) return []
  ...
  const divLine  = isDark ? 'rgba(255,255,255,0.05)' : 'rgba(0,0,0,0.06)'
  const colLine  = isDark ? 'rgba(255,255,255,0.07)' : 'rgba(0,0,0,0.08)'
  ...
  const renderTimeGrid = (days) => {
    ...
    const totalH  = (T_END - T_START) * SLOT_H
    return (
      <div className={cn('border rounded-2xl overflow-hidden flex-1 flex flex-col', card)}>
        {/* Day header row */}
        <div className="flex flex-shrink-0" style={{ borderBottom: `1px solid ${isDark ? '#27272a' : '#f3f4f6'}` }}>
          ...
          {days.map((d, i) => {
            ...
            return (
              <div key={i} className="flex-1 py-2 text-center" style={{ borderLeft: `1px solid ${colLine}` }}>
                <p className={cn('text-[10px] font-bold uppercase tracking-widest', ts)}>{DAYS[(d.getDay()+6)%7]}</p>
                <div
                  className={cn('w-7 h-7 rounded-full mx-auto flex items-center justify-center font-bold mt-0.5',
                    isToday ? 'bg-violet-600 text-white' : tp)}
                  style={{ fontSize: isDay ? '18px' : '14px' }}
                >
                  {d.getDate()}
                </div>
        ...
        <div className="overflow-y-auto flex-1" style={{ maxHeight: 540 }}>
          <div style={{ display: 'flex', height: totalH, position: 'relative' }}>
            ...
            <div style={{ position: 'absolute', inset: 0, pointerEvents: 'none', zIndex: 0 }}>
              {Array.from({ length: T_END - T_START }, (_, i) => (
                <div key={i} style={{ height: SLOT_H, borderBottom: `1px solid ${divLine}` }} />
              ))}
            </div>

            {/* Time labels */}
            <div style={{ width: '3.5rem', flexShrink: 0, zIndex: 1, position: 'relative' }}>
              {Array.from({ length: T_END - T_START }, (_, i) => (
                <div key={i} style={{ height: SLOT_H, display: 'flex', alignItems: 'flex-start', justifyContent: 'flex-end', paddingRight: '8px', paddingTop: '4px' }}>
                  <span style={{ fontSize: '10px', color: isDark ? '#52525b' : '#9ca3af' }}>
                    {String(T_START + i).padStart(2,'0')}:00
                  </span>
                </div>
              ))}
            </div>
                ...
                <div key={di} style={{
                  ...
                  {resas.map(r => {
                    ...
                    return (
                      ...
                      >
                        <p style={{ fontSize: isDay ? '12px' : '10px', fontWeight: 700, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                          {r.start_time}{r.end_time ? `–${r.end_time}` : ''} · {showClientDetails ? (r.client_name?.split(' ')[0] || '—') : r.studio}
                        </p>
                        {heightPx > 34 && (
                          <p style={{ fontSize: '9px', opacity: 0.65, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', marginTop: '1px' }}>
      ...
      <div className="flex-1 min-w-0 flex flex-col gap-3">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="flex items-center gap-1">
            ...
            <span className={cn('text-base font-bold ml-1', tp)}>{headerLabel}</span>
          </div>

          {/* View toggle */}
          <div className={cn('flex rounded-lg border overflow-hidden text-xs font-semibold', isDark ? 'border-zinc-700' : 'border-gray-200')}>
              ...
              <button
                ...
                className={cn('px-3 py-1.5 transition-colors',
                  calView === v
                    ...
                    : isDark ? 'text-zinc-400 hover:bg-zinc-800 hover:text-white' : 'text-gray-500 hover:bg-gray-50 hover:text-gray-800'
                )}
              >
                {label}
              </button>
            ...
            {studios.map(s => {
              ...
              return (
                <button
                  key={s}
                  onClick={() => setFilter(s)}
                  style={active && col ? { backgroundColor: col.light, color: col.text, borderColor: col.border } : {}}
                  className={cn(
                    'flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold border transition-all',
                    active
                      ? s === 'Tous' ? 'bg-violet-600 text-white border-violet-600' : ''
                      : isDark ? 'border-zinc-700 text-zinc-400 hover:border-zinc-500 hover:text-zinc-200' : 'border-gray-200 text-gray-500 hover:border-gray-400 hover:text-gray-700'
                  )}
                >
                  {s !== 'Tous' && <span className="w-2 h-2 rounded-full flex-shrink-0" style={{ backgroundColor: col?.bg }} />}
                  {s}
                </button>
              )
            })}
          ...
          <>
            <div className={cn('border rounded-2xl overflow-hidden flex-1', card)}>
              <div className={cn('grid grid-cols-7 border-b', divider)}>
                {DAYS.map(d => (
                  <div key={d} className={cn('py-2.5 text-center text-xs font-bold uppercase tracking-widest', ts)}>{d}</div>
                ))}
              </div>
              {grid.map((row, ri) => (
                <div key={ri} className="grid grid-cols-7" style={{ minHeight: 110 }}>
                  {row.map((cell, ci) => {
                    const resas   = getDayResas(cell)
                    const ds      = cell ? dateStr(cell) : null
                    const isToday = ds === today
                    ...
                    return (
                      <div
                        ...
                        className={cn(
                          'p-1.5 border-r border-b transition-colors', divider,
                          ...
                          <>
                            <div className="flex items-center justify-between mb-1">
                              <div className={cn('w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold',
                                isToday ? 'bg-violet-600 text-white' : isDark ? 'text-zinc-400' : 'text-gray-500')}>
                                {cell.getDate()}
                              </div>
                              {resas.length > 0 && <span className={cn('text-[9px] font-bold', ts)}>{resas.length} sess.</span>}
                            </div>
                            <div className="space-y-0.5">
                              ...
                              {resas.length > 3 && (
                                <button onClick={() => setSelected(resas[3])} className={cn('text-[10px] font-medium px-1', ts)}>
          ...
          <>
            ...
            {editing ? (() => {
              ...
              const handleSave = () => {
                ...
                const patch = {
                  date: ef.date, start_time: ef.startTime, end_time: endTime,
                  duration: Number(ef.duration), service: ef.service,
                  persons: Number(ef.persons), additional_services: ef.options, price,
                }
                      ...
                      {SERVICES.map(s => (
                        <button key={s.key} type="button" onClick={() => set('service', s.key)}
                          className={cn('flex-1 py-1.5 text-[11px] font-bold rounded-lg border transition-all',
                            ef.service === s.key ? 'bg-violet-600 border-violet-600 text-white'
                    ...
                    <div className="space-y-1">
                      {OPTIONS.map(opt => (
                        <label key={opt.key} className="flex items-center gap-2 cursor-pointer">
                          <input type="checkbox" checked={ef.options.includes(opt.key)} onChange={() => toggleOpt(opt.key)} className="w-3.5 h-3.5 accent-violet-600" />
