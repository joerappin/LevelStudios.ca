...
// ── Classic layout (admin view) — blue navy professional ─────────────────────
const C = {
  blue:       '#3b82f6',
  blueDark:   '#1d4ed8',
  blueLight:  '#93c5fd',
  ...
  headerLight:'rgba(255,255,255,0.97)',
  divider:    'rgba(255,255,255,0.07)',
  dividerL:   'rgba(15,23,42,0.08)',
  hover:      'rgba(59,130,246,0.10)',
  inactive:   'rgba(148,163,184,0.75)',
  inactiveL:  'rgba(71,85,105,0.8)',
  active:     '#3b82f6',
  activeText: '#ffffff',
}
...
export default function ClientLayout({ children, transparent = false, title }) {
  ...
  // ════════════════════════════════════════════════════════════════════════════
  // CLASSIC SIDEBAR LAYOUT — blue navy professional
  // ════════════════════════════════════════════════════════════════════════════
  if (viewMode === 'classic') {
    const pageBg    = isDark ? C.pageDark : C.page
    const hdrBg     = isDark ? C.header   : C.headerLight
    const divCol    = isDark ? C.divider  : C.dividerL
    const inactCol  = isDark ? C.inactive : C.inactiveL
    ...
    const SidebarContent = () => (
      <div style={{ display: 'flex', flexDirection: 'column', height: '100%', background: C.sidebar }}>
        ...
        <div style={{
          ...
          background: C.sidebarAlt,
          borderBottom: '1px solid rgba(255,255,255,0.06)',
        }}>
          <img src="/logo.png" style={{ width: '32px', height: '32px', objectFit: 'contain', filter: 'brightness(0) invert(1)', flexShrink: 0 }} alt="Level Studios" />
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontWeight: 800, fontSize: '13px', color: '#fff', fontFamily: 'Montserrat, sans-serif', letterSpacing: '-0.01em' }}>Level Studios</div>
            <div style={{ fontSize: '9px', fontWeight: 700, color: C.blueLight, marginTop: '1px', textTransform: 'uppercase', letterSpacing: '0.1em', opacity: 0.8 }}>Espace client</div>
          </div>
          <button onClick={toggleTheme} style={{ padding: '5px', borderRadius: '6px', border: 'none', cursor: 'pointer', background: 'transparent', color: 'rgba(255,255,255,0.35)', flexShrink: 0 }}
        ...
        {/* Client info chip */}
        <div style={{ margin: '12px 12px 4px', padding: '10px 12px', borderRadius: '10px', background: 'rgba(59,130,246,0.12)', border: '1px solid rgba(59,130,246,0.18)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '9px' }}>
            <div style={{ width: '28px', height: '28px', borderRadius: '7px', flexShrink: 0, background: C.blue, display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 800, fontSize: '12px', color: '#fff' }}>
              {user?.name?.charAt(0) || 'C'}
            </div>
            <div style={{ minWidth: 0 }}>
              <div style={{ fontSize: '12px', fontWeight: 700, color: '#fff', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{user?.name || 'Client'}</div>
              <div style={{ fontSize: '10px', color: C.blueLight, opacity: 0.75, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{user?.email}</div>
            </div>
          </div>
        </div>

        {/* Section label */}
        <div style={{ padding: '14px 20px 6px', fontSize: '9px', fontWeight: 700, letterSpacing: '0.12em', color: 'rgba(148,163,184,0.5)', textTransform: 'uppercase' }}>Navigation</div>

        {/* Nav */}
        <nav style={{ flex: 1, padding: '0 8px 12px', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '1px' }}>
            ...
            return (
              <button key={path}
                ...
                style={{
                  ...
                  color: isActive ? '#fff' : inactCol,
                }}
                onMouseEnter={e => { if (!isActive) { e.currentTarget.style.background = 'rgba(59,130,246,0.12)'; e.currentTarget.style.color = '#fff' } }}
                onMouseLeave={e => { if (!isActive) { e.currentTarget.style.background = 'transparent'; e.currentTarget.style.color = inactCol } }}
              >
                ...
                {badge && (
                  <span style={{ fontSize: '10px', padding: '1px 6px', borderRadius: '999px', fontWeight: 700, background: isActive ? 'rgba(255,255,255,0.25)' : C.blue, color: '#fff', minWidth: '18px', textAlign: 'center' }}>{badge}</span>
                )}
              </button>
            )
          })}
        ...
        {/* Bottom */}
        <div style={{ padding: '8px', flexShrink: 0, borderTop: '1px solid rgba(255,255,255,0.06)', background: C.sidebarAlt }}>
          <button onClick={handleLogout} style={{ width: '100%', display: 'flex', alignItems: 'center', gap: '10px', padding: '9px 12px', borderRadius: '8px', fontSize: '12px', fontWeight: 500, border: 'none', cursor: 'pointer', background: 'transparent', color: 'rgba(148,163,184,0.6)', transition: 'all 0.15s' }}
          ...
          >
            <LogOut size={14} />{t('logout') || 'Se déconnecter'}
          </button>
        </div>
      </div>
    )
    ...
    return (
      <div style={{ minHeight: '100vh', display: 'flex', background: pageBg }}>
        ...
        {sidebarOpen && (
          <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 40, backdropFilter: 'blur(4px)' }}
            className="lg:hidden" onClick={() => setSidebarOpen(false)} />
        )}

        <aside style={{ position: 'fixed', top: impersonatedBy ? '36px' : '0', left: 0, height: impersonatedBy ? 'calc(100% - 36px)' : '100%', width: '240px', zIndex: 50, transition: 'transform 0.3s cubic-bezier(0.4,0,0.2,1)', boxShadow: '4px 0 24px rgba(0,0,0,0.35)' }}
          className={sidebarOpen ? '' : '-translate-x-full lg:translate-x-0'}
        >
          <SidebarContent />
        </aside>

        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minHeight: '100vh' }} className="lg:ml-60">
          <header style={{ height: '60px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 24px', position: 'sticky', top: impersonatedBy ? '36px' : '0', zIndex: 30, background: hdrBg, backdropFilter: 'blur(16px)', WebkitBackdropFilter: 'blur(16px)', borderBottom: `1px solid ${divCol}`, boxShadow: isDark ? 'none' : '0 1px 4px rgba(0,0,0,0.06)' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
              <button className="lg:hidden" onClick={() => setSidebarOpen(true)} style={{ padding: '7px', borderRadius: '8px', border: 'none', cursor: 'pointer', background: 'transparent', color: inactCol }}>
                <Menu size={18} />
              </button>
              <h1 style={{ fontWeight: 700, fontSize: '16px', margin: 0, color: isDark ? '#f8fafc' : '#0f172a', fontFamily: 'Montserrat, sans-serif', letterSpacing: '-0.01em' }}>{title}</h1>
            </div>
            {activeComms.length > 0 && (
              <button onClick={() => setShowTicker(o => !o)} style={{
                ...
                border: '1px solid rgba(251,191,36,0.35)', color: '#d97706',
          ...
          {showTicker && activeComms.length > 0 && (
            <div style={{ position: 'sticky', top: impersonatedBy ? 'calc(36px + 60px)' : '60px', zIndex: 29, height: '32px', background: isDark ? 'rgba(251,191,36,0.07)' : 'rgba(251,191,36,0.10)', borderBottom: `1px solid ${isDark ? 'rgba(251,191,36,0.15)' : 'rgba(217,119,6,0.2)'}`, display: 'flex', alignItems: 'center', overflow: 'hidden' }}>
              <div style={{ whiteSpace: 'nowrap', fontSize: '12px', fontWeight: 500, color: '#d97706', animation: `commTicker ${tickerDuration}s linear infinite` }}>
                {tickerText}
              </div>
              <button onClick={() => setShowTicker(false)} style={{ position: 'absolute', right: 8, background: 'transparent', border: 'none', color: 'rgba(217,119,6,0.5)', cursor: 'pointer', padding: 2 }}>
                <XIcon size={13} />
              </button>
            </div>
          )}

          <main style={{ flex: 1, padding: '28px', color: isDark ? '#f1f5f9' : '#0f172a' }}>
            {children}
          </main>
        </div>
      </div>
