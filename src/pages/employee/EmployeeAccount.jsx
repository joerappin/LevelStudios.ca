...
export default function EmployeeAccount() {
  ...
  const handlePasswordSave = (e) => {
    ...
    // Update in ls_accounts if the account exists there
  ...
  return (
    <Layout navItems={EMPLOYEE_NAV} title="Profil">
      <div className="space-y-5 max-w-2xl">
        <h2 className={`text-2xl font-bold ${textPrimary}`}>Profil</h2>
          ...
          <div className="p-6">
            {pwSuccess ? (
              <div className="flex flex-col items-center gap-3 py-8 text-center">
                ...
                <p className={`font-bold ${textPrimary}`}>Mot de passe mis à jour !</p>
                <p className={`text-sm ${textSecondary}`}>Votre mot de passe a été modifié avec succès.</p>
              </div>
            ) : (
              <form onSubmit={handlePasswordSave} className="space-y-4 max-w-sm">
                {[
                  ...
                  <div key={f.key}>
                    <label className={`block text-sm font-medium mb-1.5 ${textPrimary}`}>{f.label}</label>
                    <div className="relative">
                      ...
                      />
                      <button type="button" onClick={f.toggle} className={`absolute right-3 top-1/2 -translate-y-1/2 ${textSecondary}`}>
